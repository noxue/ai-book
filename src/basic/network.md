...
## 通过 TCP协议 收发数据，演示数据的传输过程

#### 1. 电脑A 和 电脑B 通过 3次握手 建立 TCP 连接
 
只要知道发送数据之前需要先建立连接，具体细节可以ai问：tcp 3次握手是什么

#### 2. 电脑A 的某个程序 对 电脑B 的某个程序 准备发送数据

数据从应用层，也就是电脑A中的某个程序发出

![tcp 发送数据](./images/network-04.png)

#### 3. 数据从应用层发出，经过传输层

首先发送给电脑系统，来到传输层，传输层会加上 TCP 头。

TCP 头中包含源端口和目标端口，源端口是电脑A的程序使用的端口，目标端口是电脑B的程序的端口。

![tcp 发送数据](./images/network-05.png)

#### 4. 经过网络层

网络层会加上 IP 头，IP 头中包含源 IP 地址和目标 IP 地址。

源 IP 地址是电脑A的 IP 地址，目标 IP 地址是电脑B的 IP 地址。

![tcp 发送数据](./images/network-06.png)

#### 5. 经过链路层

数据链路层 就不画了，最终通过网络传输到远程电脑

#### 6. 远程电脑的收到数据包来到网络层

远程电脑收到数据后，来到网络层。会去掉 IP 头，然后发给传输层。

![tcp 发送数据](./images/network-07.png)

#### 7. 远程电脑的收到数据包来到传输层

远程电脑收到数据后，来到传输层。会去掉 TCP 头，然后发给应用层。

![tcp 发送数据](./images/network-08.png)


#### 8. 远程电脑的收到数据包来到应用层

远程电脑收到数据后，来到应用层。会去掉应用层的数据，得到原始数据。

![tcp 发送数据](./images/network-09.png)



#### 9. 如果需要断开连接

电脑A 和 电脑B 通过 4次挥手 断开 TCP 连接

想具体细节可以ai问：tcp 4次挥手是什么
